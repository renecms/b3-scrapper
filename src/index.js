...
const cheerio = require('cheerio');
 ...
 async function getLatestEvents() {
    try {
        ...
        $('#ctl00_contentPlaceHolderConteudo_grdBonificacao_ctl01 > tbody > tr').each((_idx, row) => {
            $columns = $(row).find('td');
            currentBonus = {}
        ...
        $('#ctl00_contentPlaceHolderConteudo_grdDividendo_ctl01 > tbody > tr').each((_idx, row) => {
            $columns = $(row).find('td');
...
async function getAllDividends() {
    try {
        const dividendList = []
        const { data } = await axios.get(
            'http://bvmf.bmfbovespa.com.br/cias-listadas/empresas-listadas/ResumoProventosDinheiro.aspx?codigoCvm=19348&tab=3.1&idioma=pt-br'
        );
        const $ = cheerio.load(data);

        $('#ctl00_contentPlaceHolderConteudo_grdProventos_ctl01 > tbody > tr').each((_idx, row) => {
            $columns = $(row).find('td');
            const currentDividend = {}
